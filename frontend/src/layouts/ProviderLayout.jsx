...
import {
  ...
  LogOut, Bell, ChevronDown, User,
} from 'lucide-react';
import { useAuth } from '../context/AuthContext';

const PAGE_TITLES = {
  '/provider/dashboard': 'Dashboard',
  '/provider/capture':   'Capture Reading',
  '/provider/patients':  'Patient Records',
};

const NAV = [
  { icon: LayoutDashboard, label: 'Dashboard',       to: '/provider/dashboard', end: true  },
  { icon: Camera,          label: 'Capture Reading', to: '/provider/capture',   end: true  },
      ...
      <aside className="w-64 bg-navy-900 flex flex-col flex-shrink-0">
        ...
        <div className="flex items-center gap-3 px-6 py-5 border-b border-navy-800">
          ...
          <div>
            ...
            <p className="text-navy-600 text-[11px] mt-0.5 font-medium tracking-wide">Health Provider</p>
          </div>
        </div>
        ...
        <div className="border-t border-navy-800 p-4">
          <div className="flex items-center gap-3 mb-3">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-white text-sm font-semibold truncate">{user?.name || 'Provider'}</p>
              <p className="text-slate-500 text-xs">Healthcare Provider</p>
            </div>
          </div>
          <button
        ...
        <header className="bg-white border-b border-gray-200 px-6 py-3 flex items-center gap-4 flex-shrink-0">
          ...
          <div className="relative" ref={menuRef}>
            ...
            >
              ...
              <span className="hidden md:block font-medium truncate max-w-[100px]">{user?.name || 'Provider'}</span>
